...
import { type UpdateCouponInput, updateCouponSchema } from "./schema";

type Result = { success: boolean; message: string };

export async function updateCoupon(input: UpdateCouponInput): Promise<Result> {
  await requireAdminSessionOrThrow();

  const payload = updateCouponSchema.safeParse(input);
  ...
  const { couponId, ...data } = payload.data;
