...
export async function registerAdminUser(
  ...
  if (!isAdminRegistrationConfigured())
    return {
      success: false,
      message: "O cadastro administrativo ainda não foi configurado no ambiente.",
    };
  ...
  if (!adminRole)
    return {
      success: false,
      message: "Este e-mail não possui permissão para cadastro administrativo.",
    };
  ...
  try {
    ...
    return { success: true, message: "Conta admin criada. Faça login para continuar." };
  } catch (error) {
