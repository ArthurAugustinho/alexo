...
function getErrorMessage(error: unknown) {
  ...
  return "NÃ£o foi possÃ­vel criar a conta administrativa.";
}
...
export async function registerAdminUser(
  ...
  if (!isAdminRegistrationConfigured()) {
    return {
      success: false,
      message:
        "O cadastro administrativo ainda nÃ£o foi configurado no ambiente.",
    };
  }
  ...
  if (!adminRole) {
    return {
      success: false,
      message:
        "Este e-mail nÃ£o possui permissÃ£o para cadastro administrativo.",
    };
  }
  ...
  try {
    ...
    return {
      success: true,
      message: "Conta admin criada. FaÃ§a login para continuar.",
    };
  } catch (error) {
